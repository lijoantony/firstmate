...
When a selected delivery path calls for a diff, `bin/fm-review-diff.sh` resolves that same delivery target branch as its base and, when task meta records `pr=`, always fetches and compares against `refs/pull/<n>/head` by default (recorded `pr_head=` is only an offline fallback) before falling back to the local branch with a warning.
It refreshes that base from origin when origin carries the branch, reviews against the local branch when a reachable origin does not carry it - the ordinary shape for work stacked on a branch its own rules forbid pushing - and refuses outright when origin is unreachable, because a review against a base that may lag presents other people's commits as this task's work.
